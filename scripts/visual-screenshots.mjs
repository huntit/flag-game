...
async function shot(name, contextOptions, fn) {
  ...
  const notes = await page.evaluate(() => ({
    ...
    goalSquares: document.querySelectorAll('.goal-square').length,
    bagCount: document.querySelector('.market-bag-count')?.textContent ?? null,
    tileSize: document.querySelector('.rack-tile')?.getBoundingClientRect().width,
